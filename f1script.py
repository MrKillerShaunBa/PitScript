...
def start(code, parent_vars = None, funcs = None):
    ...
    i=0
    while i < len(tokens):
        if i==0 and tokens[i] != "its lights out and away we go":
            raise SyntaxError("Race hasnt started bro, use - 'its lights out and away we go'")
        
        elif tokens[i].startswith("strat "):
            ...
            body = []
            i+=1
            while i < len(tokens) and tokens[i] != "green":
                body.append(tokens[i])
                i += 1
            if i >= len(tokens) or tokens[i] != "green":
                raise SyntaxError("Add box to end pit stop")
            ...
            if message.startswith("grid"):
                ...
                if message in variables.keys():
                    ...
                    print(message)
            # else:
            #     raise ValueError(f"Unknown radio message: {message}")
        elif i == len(tokens)-1 and tokens[i] != "retire the car":
